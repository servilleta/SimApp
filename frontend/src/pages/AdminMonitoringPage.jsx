...
const AdminMonitoringPage = () => {
  ...
  const fetchMonitoringData = async (isInitialLoad = false) => {
    ...
    // Fetch system monitoring (Server 1 stress, Paperspace status)
    try {
      console.log('🖥️ Fetching system monitoring data...');
      const systemResponse = await axios.get(`/api/admin/monitoring/system/monitoring`, {
        headers: { Authorization: `Bearer ${getToken()}` }
      });
      console.log('✅ System monitoring data received:', systemResponse.data);
      setSystemMonitoring(systemResponse.data);
    } catch (e) {
      console.error('❌ Failed to fetch system monitoring:', e);
      setSystemMonitoring(null);
    }
  ...
  useEffect(() => {
    ...
    // Refresh every 30 seconds (subsequent loads)
    const interval = setInterval(() => fetchMonitoringData(false), 30000);
  ...
  return (
    <>
          ...
          {isRefreshing && (
            <div style={{
              ...
              <div style={{
                width: '12px',
                height: '12px',
                border: '2px solid var(--color-braun-orange)30',
                borderTop: '2px solid var(--color-braun-orange)',
                borderRadius: '50%',
      ...
      {/* Server 1 Stress Monitoring & Paperspace Status */}
      {systemMonitoring && (
        <div style={{ marginBottom: '3rem' }}>
          <div style={{ 
            display: 'flex', 
            ...
            <div style={{
              width: '60px',
              height: '60px',
              background: 'linear-gradient(135deg, var(--color-braun-orange)15, var(--color-braun-orange)25)',
              borderRadius: '20px',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
            ...
            <div>
              <h2 style={{ 
                ...
                System Monitoring & Auto-Scaling
              </h2>
              <p style={{ 
                color: 'var(--color-medium-grey)', 
                margin: 0,
                ...
                Server 1 stress levels and Paperspace auto-scaling triggers
                {systemMonitoring && (
                  <span style={{ 
                    marginLeft: '1rem', 
                    fontSize: '0.8rem',
                    ...
                    • Last updated: {new Date(systemMonitoring.system_metrics.timestamp).toLocaleTimeString()}
                  </span>
                )}
              </p>
            </div>
          ...
          <div style={{
            ...
            <MetricCard
              icon="🔥"
              title="CPU Usage"
              value={systemMonitoring.scaling_metrics.current_cpu}
              unit="%"
              status={systemMonitoring.scaling_metrics.current_cpu > systemMonitoring.scaling_metrics.cpu_threshold ? 'warning' : 'healthy'}
              color={systemMonitoring.scaling_metrics.current_cpu > systemMonitoring.scaling_metrics.cpu_threshold ? 'var(--color-error)' : 'var(--color-success)'}
            />
            ...
            <MetricCard
              icon="🧠"
              title="Memory Usage"
              value={systemMonitoring.scaling_metrics.current_memory}
              unit="%"
              status={systemMonitoring.scaling_metrics.current_memory > systemMonitoring.scaling_metrics.memory_threshold ? 'warning' : 'healthy'}
              color={systemMonitoring.scaling_metrics.current_memory > systemMonitoring.scaling_metrics.memory_threshold ? 'var(--color-error)' : 'var(--color-success)'}
            />
            ...
            <MetricCard
              icon="⚡"
              title="API Response"
              value={systemMonitoring.scaling_metrics.current_response_time}
              unit="ms"
              status={systemMonitoring.scaling_metrics.current_response_time > systemMonitoring.scaling_metrics.response_time_threshold ? 'warning' : 'healthy'}
              color={systemMonitoring.scaling_metrics.current_response_time > systemMonitoring.scaling_metrics.response_time_threshold ? 'var(--color-error)' : 'var(--color-success)'}
            />
            ...
            <MetricCard
              ...
              value={systemMonitoring.scaling_metrics.current_simulations}
              unit=""
              status={systemMonitoring.scaling_metrics.current_simulations > systemMonitoring.scaling_metrics.active_simulations_threshold ? 'warning' : 'healthy'}
              color={systemMonitoring.scaling_metrics.current_simulations > systemMonitoring.scaling_metrics.active_simulations_threshold ? 'var(--color-warning)' : 'var(--color-success)'}
            />
          </div>
          ...
          <div style={{
            ...
            <div className="card" style={{ padding: '1.5rem' }}>
              <h3 style={{ margin: '0 0 1rem 0', color: 'var(--color-dark-grey)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                {systemMonitoring.paperspace_status.server1_running ? '🟢' : '🔴'} Server 1 (Primary)
              </h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span>Status:</span>
                  <span style={{ color: systemMonitoring.paperspace_status.server1_running ? 'var(--color-success)' : 'var(--color-error)' }}>
                    {systemMonitoring.paperspace_status.server1_running ? 'RUNNING' : 'STOPPED'}
                  </span>
                </div>
                {systemMonitoring.paperspace_status.server1_ip && (
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span>IP Address:</span>
                    <span>{systemMonitoring.paperspace_status.server1_ip}</span>
                  </div>
                )}
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span>Type:</span>
            ...
            <div className="card" style={{ padding: '1.5rem' }}>
              <h3 style={{ margin: '0 0 1rem 0', color: 'var(--color-dark-grey)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                {systemMonitoring.paperspace_status.server2_running ? '🔥' : '💤'} Server 2 (Performance)
              </h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span>Status:</span>
                  <span style={{ color: systemMonitoring.paperspace_status.server2_running ? 'var(--color-success)' : 'var(--color-medium-grey)' }}>
                    {systemMonitoring.paperspace_status.server2_running ? 'RUNNING' : 'STANDBY'}
                  </span>
                </div>
                {systemMonitoring.paperspace_status.server2_ip && (
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span>IP Address:</span>
                    <span>{systemMonitoring.paperspace_status.server2_ip}</span>
                  </div>
                )}
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span>Type:</span>
                  <span>Ampere A4000</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span>Cost:</span>
                  <span>$0.76/hour</span>
          ...
          <div className="card" style={{ 
            padding: '1.5rem', 
            marginBottom: '1.5rem',
            background: systemMonitoring.scaling_metrics.scaling_recommendation === 'SCALE_UP_RECOMMENDED' ? 'linear-gradient(135deg, var(--color-warning)05, var(--color-warning)10)' : 'var(--color-background-secondary)'
          }}>
            <h3 style={{ margin: '0 0 1rem 0', color: 'var(--color-dark-grey)' }}>
              🤖 Auto-Scaling Recommendation
            </h3>
            <div style={{ 
              fontSize: '1.1rem', 
              fontWeight: '600',
              color: systemMonitoring.scaling_metrics.scaling_recommendation === 'SCALE_UP_RECOMMENDED' ? 'var(--color-warning)' : 'var(--color-success)',
              marginBottom: '1rem'
            }}>
              {systemMonitoring.scaling_metrics.scaling_recommendation.replace(/_/g, ' ')}
            </div>
            
            {/* Active Scaling Triggers */}
            {systemMonitoring.paperspace_status.scaling_triggers_active.length > 0 && (
              <div style={{ marginBottom: '1rem' }}>
                <strong>Active Triggers:</strong>
                <ul style={{ margin: '0.5rem 0', paddingLeft: '1.5rem' }}>
                  {systemMonitoring.paperspace_status.scaling_triggers_active.map((trigger, idx) => (
                    <li key={idx} style={{ color: 'var(--color-error)' }}>{trigger}</li>
                  ))}
                </ul>
              </div>
            )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem', fontSize: '0.9rem' }}>
              <div>
                <span style={{ color: 'var(--color-medium-grey)' }}>Current:</span>
                <div style={{ fontWeight: '600' }}>${systemMonitoring.scaling_metrics.cost_impact.current_hourly}/hr</div>
              </div>
              <div>
                <span style={{ color: 'var(--color-medium-grey)' }}>If Scaled:</span>
                <div style={{ fontWeight: '600' }}>${systemMonitoring.scaling_metrics.cost_impact.scaled_hourly}/hr</div>
              </div>
              <div>
                <span style={{ color: 'var(--color-medium-grey)' }}>Daily Impact:</span>
                <div style={{ fontWeight: '600' }}>${systemMonitoring.scaling_metrics.cost_impact.daily_if_scaled}</div>
              </div>
            </div>
          </div>

          {/* Alerts */}
          {systemMonitoring.alerts && systemMonitoring.alerts.length > 0 && (
            <div className="card" style={{ padding: '1.5rem', marginBottom: '1.5rem' }}>
              <h3 style={{ margin: '0 0 1rem 0', color: 'var(--color-dark-grey)' }}>🚨 System Alerts</h3>
              {systemMonitoring.alerts.map((alert, idx) => (
                <div key={idx} style={{
                  padding: '0.75rem',
                  marginBottom: '0.5rem',
                  borderRadius: '8px',
                  background: alert.level === 'critical' ? 'var(--color-error)10' : 
                            alert.level === 'warning' ? 'var(--color-warning)10' : 'var(--color-info)10',
                  borderLeft: `4px solid ${alert.level === 'critical' ? 'var(--color-error)' : 
                                         alert.level === 'warning' ? 'var(--color-warning)' : 'var(--color-info)'}`
                }}>
                  <div style={{ fontWeight: '600', marginBottom: '0.25rem' }}>{alert.message}</div>
                  <div style={{ fontSize: '0.9rem', color: 'var(--color-medium-grey)' }}>{alert.action}</div>
                </div>
              ))}
            </div>
          )}
          ...
          {/* Recommendations */}
          {systemMonitoring.recommendations && systemMonitoring.recommendations.length > 0 && (
            <div className="card" style={{ padding: '1.5rem' }}>
              <h3 style={{ margin: '0 0 1rem 0', color: 'var(--color-dark-grey)' }}>💡 Optimization Recommendations</h3>
              <ul style={{ margin: 0, paddingLeft: '1.5rem' }}>
                {systemMonitoring.recommendations.map((rec, idx) => (
                  <li key={idx} style={{ marginBottom: '0.5rem', color: 'var(--color-dark-grey)' }}>{rec}</li>
                ))}
              </ul>
            </div>
          )}
