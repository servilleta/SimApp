...
function useMarkdown(url) {
  ...
  useEffect(() => {
    fetch(url)
      .then(r => r.text())
      .then(setContent)
      .catch(() => setContent('Failed to load acceptable use policy.'));
  }, [url]);
  return content;
}
...
function AcceptableUsePage() {
  ...
  const aupText = useMarkdown('/legal/ACCEPTABLE_USE_POLICY.md');
    ...
    <div style={styles.container}>
      ...
      <div style={styles.content}>
        <div style={styles.markdownContent}>
          <ReactMarkdown>{aupText}</ReactMarkdown>
        </div>
      </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div>
              ...
              <button onClick={() => navigate('/acceptable-use')} style={{ background: 'none', border: 'none', color: 'var(--color-braun-orange)', cursor: 'pointer', padding: 0, marginBottom: '8px', display: 'block' }}>Acceptable Use</button>
