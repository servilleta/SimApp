  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchSimulationStatus.rejected, (state, action) => {
        // Don't mark as failed if we already have completed results OR progress shows completion
        ...
        const progressShowsCompletion = state.progressData.progressPercentage >= 100 || 
                                       state.progressData.currentStage === 'results' ||
                                       state.progressData.hasCompleted;
        
        // CRITICAL FIX: Don't mark as failed if we have ANY progress data
        const hasAnyProgress = state.progressData.progressPercentage > 0 || 
                              state.progressData.currentStage !== 'unknown' ||
                              state.progressData.currentIteration > 0;
        
        if (!hasCompletedResults && !progressShowsCompletion && !hasAnyProgress && state.status !== 'completed') {
          state.status = 'failed';
          state.error = action.payload;
          console.log('[simulationSlice] ❌ SIMULATION FAILED: No completion indicators found');
        } else {
          // If progress shows completion, trust that over status endpoint failure
          if (progressShowsCompletion && state.status !== 'completed') {
            state.status = 'completed';
            state.currentStage = 'results';
            console.log('[simulationSlice] 🎯 PROGRESS-OVERRIDE: Status endpoint failed but progress shows completion - marking as completed');
          } else if (hasAnyProgress && state.status === 'idle') {
            // If we have progress data, keep status as running instead of failing
            state.status = 'running';
            console.log('[simulationSlice] 🔄 PROGRESS-BASED STATUS: Status endpoint failed but progress data exists - keeping as running');
          }
      ...
      .addCase(fetchSimulationProgress.fulfilled, (state, action) => {
        ...
        console.log(`[simulationSlice] 🔍 PROGRESS DEBUG: Received progress data:`, {
          simulationId: progressData.simulationId,
          normalizedId,
          currentParentId: state.currentParentSimulationId,
          isForCurrentParent,
          ...
          if (isForCurrentParent) {
            ...
            // CRITICAL FIX: Prevent erratic progress bar behavior
            // Never allow progress to go backwards unless it's a new simulation or stage change
            const isStageChange = progressData.stage !== state.progressData.currentStage;
            const isNewSimulation = !state.progressData.timestamp || Date.now() - state.progressData.timestamp > 60000;
            const isCompletionUpdate = newProgressPercentage >= 100 || progressData.stage === 'results';
            
            const allowProgressUpdate = (!isDecrease) || isNewSimulation || isStageChange || isCompletionUpdate;
            
            // CRITICAL FIX: Define hasCompleted OUTSIDE the if block to prevent ReferenceError
            const hasCompleted = newProgressPercentage >= 100 || progressData.stage === 'results';
            
            if (allowProgressUpdate) {
              
              console.log(`[simulationSlice] ✅ UPDATING PROGRESS DATA:`, {
                newProgressPercentage,
                currentProgressPercentage,
                stage: progressData.stage,
                hasCompleted,
                currentIteration: progressData.current_iteration,
                totalIterations: progressData.total_iterations,
                isDecrease,
                allowedReason: !isDecrease ? 'progress_increase' : isNewSimulation ? 'new_simulation' : isStageChange ? 'stage_change' : isCompletionUpdate ? 'completion' : 'unknown'
              });
            } else {
              console.log(`[simulationSlice] ❌ REJECTED PROGRESS UPDATE (preventing erratic behavior):`, {
                newProgressPercentage,
                currentProgressPercentage,
                stage: progressData.stage,
                isDecrease,
                ...
                isNewSimulation,
                isCompletionUpdate,
                reason: 'progress_would_go_backwards'
              });
              // Don't update progress data when rejecting backwards progress
              return;
            }
            ...
            // CRITICAL FIX: If progress shows completion, update simulation status
            if (hasCompleted && state.status !== 'completed') {
              state.status = 'completed';
              state.currentStage = 'results';
              console.log(`[simulationSlice] 🎯 PROGRESS-BASED COMPLETION: Setting status to 'completed' based on progress data (${newProgressPercentage}%, stage: ${progressData.stage})`);
            }
          }
        }
      });
