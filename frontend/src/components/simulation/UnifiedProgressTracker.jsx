...
const UnifiedProgressTracker = ({ simulationIds = [], childSimulationIds = [], targetVariables = [], forceCompleted = false, primaryEngineType = null, restoredResults = [], onResultsCheck = null }) => {
  ...
  useEffect(() => {
    ...
    const shouldStartPolling = isActive && shouldPoll && primarySimulationId && 
      ...
      progressDataFromStore.currentStage !== 'results' &&
      progressDataFromStore.progressPercentage < 100;
    ...
    if (shouldStartPolling) {
      ...
      }, 3000); // Poll every 3 seconds (reduced frequency to prevent race conditions)
  ...
  useEffect(() => {
    if (progressDataFromStore && primarySimulationId && isActive) {
      ...
      if (progressDataFromStore.progressPercentage !== undefined && progressDataFromStore.progressPercentage >= 0) {
        ...
        // Handle completion based on progress percentage OR completion status
        const isCompleted = progressDataFromStore.progressPercentage >= 100 || 
                           progressDataFromStore.currentStage === 'results' ||
                           globalStatus === 'completed';
        ...
        if (isCompleted) {
          ...
          // Stop polling immediately when completed
          if (pollingIntervalRef.current) {
            clearInterval(pollingIntervalRef.current);
            pollingIntervalRef.current = null;
            setIsPolling(false);
            logger.debug('[UnifiedProgressTracker] 🛑 Stopped progress polling - simulation completed');
          }
          
          // Ensure progress shows 100% for completed simulations
          if (progressDataFromStore.progressPercentage < 100 && globalStatus === 'completed') {
            setUnifiedProgress(prev => ({
              ...prev,
              overallProgress: 100,
