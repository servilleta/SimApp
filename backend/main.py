...
# Infrastructure Management API for Paperspace integration
try:
    from routers.infrastructure import router as infrastructure_router
    app.include_router(infrastructure_router, prefix=settings.API_PREFIX, tags=["🏗️ Infrastructure Management"])
    logger.info("✅ Infrastructure management router included successfully - PAPERSPACE API INTEGRATION ACTIVE")
except ImportError as e:
    logger.warning(f"⚠️ Infrastructure management router not available: {e}")
except Exception as e:
    logger.error(f"❌ Failed to include infrastructure management router: {e}")
